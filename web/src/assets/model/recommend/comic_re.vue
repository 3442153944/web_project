    ...
    <div class="comic_recommend">
        ...
        <div class="content">
            <div class="item" v-for="(item, index) in data" :key="index">
                <router-link :to="`/comic_content?id=${item.id}`">
...
const get_data = async () => {
    ...
    if (res.status == 200) {
        ...
        for (let i = 0; i < data.value.length; i++) {
            let user_info = (await api.get_user_info(data.value[i].belong_to_userid))
...
@media (max-width: 1024px) {
    .content {
        grid-template-columns: repeat(auto-fcomic, minmax(250px, 1fr));
    }
}

@media (max-width: 768px) {
    .content {
        grid-template-columns: repeat(auto-fcomic, minmax(200px, 1fr));
        gap: 15px;
    }
}
